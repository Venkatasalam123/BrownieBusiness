...
SERVICE_ACCOUNT_FILE = 'credentials.json'  # Service account JSON file
...
class GoogleSheetsDB:
    ...
    def _connect(self):
        ...
        try:
            # Try service account first (recommended for server-side)
            if os.path.exists(SERVICE_ACCOUNT_FILE):
                creds = service_account.Credentials.from_service_account_file(
                    SERVICE_ACCOUNT_FILE, scopes=SCOPES)
            else:
                # Fallback to OAuth (for user authentication)
                # You'll need to set up OAuth flow if using this
                error_msg = (
                    f"Service account file '{SERVICE_ACCOUNT_FILE}' not found.\n\n"
                    "To use Google Sheets:\n"
                    "1. Follow instructions in SETUP_GOOGLE_SHEETS.md\n"
